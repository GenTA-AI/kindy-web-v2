...
readonly RUNTIME_SERVICE_ACCOUNT="kindy-runtime@${PROJECT_ID}.iam.gserviceaccount.com"
...
readonly -a PREVIEW_REQUIRED_SECRET_BINDINGS=(
  "SUPABASE_SERVICE_ROLE_KEY=kindy-preview-supabase-service-role:latest"
)

readonly -a PREVIEW_OPTIONAL_SECRET_BINDINGS=(
  "ANTHROPIC_API_KEY=kindy-anthropic-key:latest"
  "FAL_KEY=kindy-fal-key:latest"
  "GOOGLE_API_KEY=kindy-google-key:latest"
)

readonly -a PRODUCTION_REQUIRED_SECRET_BINDINGS=(
  "SUPABASE_SERVICE_ROLE_KEY=kindy-supabase-service-role-key:latest"
  "ANTHROPIC_API_KEY=kindy-anthropic-key:latest"
  "FAL_KEY=kindy-fal-key:latest"
  "GOOGLE_API_KEY=kindy-google-key:latest"
  "INNGEST_SIGNING_KEY=kindy-inngest-signing-key:latest"
  "INNGEST_EVENT_KEY=kindy-inngest-event-key:latest"
  "PORTONE_API_SECRET=kindy-portone-api-secret:latest"
  "PORTONE_WEBHOOK_SECRET=kindy-portone-webhook-secret:latest"
  "BILLING_KEY_SECRET=kindy-billing-key-secret:latest"
  "KINDY_OPERATOR_KEY=kindy-operator-key:latest"
)

readonly -a PRODUCTION_OPTIONAL_SECRET_BINDINGS=(
  "TOSS_SECRET_KEY=kindy-toss-secret-key:latest"
  "RESEND_API_KEY=kindy-resend-api-key:latest"
  "RESEND_FROM_EMAIL=kindy-resend-from-email:latest"
)
...
Run receives Secret Manager references only.
USAGE
}
...
secret_name_from_binding() {
  local binding="$1"
  local secret_version="${binding#*=}"
  printf '%s' "${secret_version%:*}"
}
...
secret_version_exists() {
  ...
  gcloud secrets versions describe latest \
    --secret="$secret_name" \
    --project="$PROJECT_ID" \
    --format='value(state)' 2>/dev/null | grep -qx 'ENABLED'
}
...
runtime_secret_bindings() {
  ...
  case "$profile" in
    preview)
      required=("${PREVIEW_REQUIRED_SECRET_BINDINGS[@]}")
      optional=("${PREVIEW_OPTIONAL_SECRET_BINDINGS[@]}")
      ;;
    production)
...
smoke_canary() {
  ...
  for ((attempt = 1; attempt <= attempts; attempt += 1)); do
    ...
    if [[ "$body" = *"$expected_sha"* ]]; then
      log "canary smoke observed release $expected_sha on attempt $attempt/$attempts"
      return 0
    fi
  done

  printf '[kindy-release] ERROR: canary never reported expected release SHA %s\n' \
    "$expected_sha" >&2
  return 1
}
...
deploy_revision() {
  ...
  gcloud run deploy "$service" \
    ...
    --update-env-vars="KINDY_DEPLOY_ENV=${deploy_env},KINDY_RELEASE_SHA=${sha},KINDY_LOCAL_PREVIEW=0,LESSON_GUEST_MODE=0" \
    --remove-env-vars=VERCEL_ENV,INNGEST_DEV \
...
bootstrap() {
  ...
  if ! gcloud iam service-accounts describe "$RUNTIME_SERVICE_ACCOUNT" \
    ...
    log "creating least-privilege Cloud Run runtime service account"
    gcloud iam service-accounts create kindy-runtime \
      --project="$PROJECT_ID" \
      --display-name='Kindy Cloud Run runtime' \
      --description='Runtime identity for Kindy preview and production services' \
      --quiet
  fi
  ...
  for binding in \
    "${PREVIEW_REQUIRED_SECRET_BINDINGS[@]}" \
    "${PREVIEW_OPTIONAL_SECRET_BINDINGS[@]}" \
...
deploy_preview() {
  ...
  assert_clean_at_sha "$sha"
  assert_artifact_repository
  assert_image_matches_sha "$digest_uri" "$sha" either

  log "deploying preview from exact digest $digest_uri"
  deploy_revision "$PREVIEW_SERVICE" preview "$digest_uri" "$sha" traffic
  ...
  smoke_url "$url" "$sha" false || die "preview smoke failed; production promotion is blocked"
...
promote_production() {
  ...
  smoke_url "$preview_url" "$sha" false || die "preview readiness no longer passes; promotion blocked"
  ...
  if ! smoke_canary "$PRODUCTION_URL" "$sha" 80; then
  ...
  for percent in 25 50; do
    ...
    gcloud run services update-traffic "$PRODUCTION_SERVICE" \
      ...
      --quiet
    if ! smoke_canary "$PRODUCTION_URL" "$sha" 24; then
  ...
  if ! smoke_url "$PRODUCTION_URL" "$sha" false; then
