...
def smoke(base):
    for path,needle in [('/','주식회사 젠타'),('/review/payment','결제 경로 데모'),('/legal/refund','남아 있는 미이용 기간'),('/legal/business','주식회사 젠타'),('/auth/login','로그인')]:
        code,body=http(base+path)
        assert code==200 and needle in body,(path,code)
    code,body=http(base+'/api/payments/toss/billing-key','POST')
    assert code==503 and 'billing_not_ready' in body,('billing must remain disabled',code)
    code,body=http(base+'/review/payment/result?authKey=fake&customerKey=fake')
    assert code==200 and '결제 성공 증빙이 아닙니다' in body
...
if mode=='candidate':
    ...
    record=json.loads((OUT/'release.json').read_text());assert record.get('candidateSmokePassed')
    current=service()
