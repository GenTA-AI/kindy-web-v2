...
## presale-lockdown 킥오프 결정 (2026-08-03, 대표 면담)
- **런칭 프레임 = 실가 프리세일 우선**(07-21 재스코프 C 계승). 공개 유료 MVP는 NO-GO 유지.
  근거 문서 = `claudecodex.md` Part II(Codex 감사 9도메인 독립 재검증본).
- **미션 범위 = 잠그기 트랙만.** 프리세일 결제 라우트 신규 구현과 카피·법무 재작성은 후속 미션.
- **환불 정본 = 14일 100% 보장 유지.** 랜딩·프로모·PG 가맹심사에 이미 나간 약속을 정본으로 삼고,
  약관 §7과 체크아웃(현재 법정 7일·미사용)을 여기에 맞춘다. **법무 검토 선행 → 후속 미션.**
- **카톡 = 채널 개설 전제로 카피 유지.** 배송·해지 약속을 그대로 두되 8/31 전 채널 개설 +
  알림톡 템플릿 심사가 완료되어야 한다. 심사 지연 시 약속 위반 상태로 결제를 받게 되므로 운영 리스크.
- **랜딩 클레임: 이름 호명·"여섯 달 마흔여덟 편"만 제거, 교수·런던·경쟁사 비교표는 유지**(대표 판단).
  이름 호명은 기능 자체가 없다(gemini-tts 한글 이름 오독). 48편은 재고 1편 대비 과대 약속.
- **island-polish 미션 종료**(RETRO 작성). 게임 표면은 07-21 폴리싱 동결 유지.
