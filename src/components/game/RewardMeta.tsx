...
const STICKER_META: Record<string, { mark: string; label: string; className: string }> = {
  ...
  challenge: {
    mark: '◆',
    label: '새 놀이 성공',
    className: 'border-sagebg bg-sagebg text-saged',
  },
  steady_try: {
    mark: '↺',
    label: '다시 해낸 힘',
...
export default function RewardMeta({ state, lastDelta = null, totalSlots }: RewardMetaProps) {
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-line bg-white p-4 shadow-sm shadow-sagebg"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold text-sage">오늘 모은 기록</p>
        ...
        <div
          className={`flex min-h-[44px] items-center gap-2 rounded-2xl border px-4 py-2 transition-all duration-300 ${
            newStars > 0
              ? 'scale-105 border-sagebg bg-sagebg text-saged'
        ...
        >
          ...
          <div>
            <p className="text-xs font-medium text-gray-500">모은 빛</p>
      ...
      {hasNewReward && (
        <div className="mt-4 flex flex-wrap gap-2">
          {newStars > 0 && (
            <span className="inline-flex min-h-[44px] items-center rounded-full bg-mist px-4 text-sm font-bold text-saged transition-all duration-300">
              새 빛 +{newStars}
            </span>
          )}
      ...
      <div className="mt-5 rounded-2xl bg-sagebg p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-bold text-ink">이야기 꽃</p>
            <p className="mt-1 text-xs font-medium text-saged">
              {safeTotalSlots > 0 ? `${filledSlots}/${safeTotalSlots}칸` : '새 이야기 준비 중'} · {roundedProgress}%
            </p>
          </div>
          <span className="inline-flex min-h-[44px] min-w-[64px] items-center justify-center rounded-full bg-white px-3 text-sm font-extrabold text-saged shadow-sm">
            {roundedProgress}%
          </span>
        </div>
        ...
        <p className="mt-3 text-sm font-semibold text-saged">
          {roundedProgress >= 100
            ? '이야기 꽃을 모두 피웠어요. 다음 이야기는 보호자와 함께 골라요.'
            : `이야기 꽃이 ${roundedProgress}% 피었어요. 다음 이야기까지 조금만 더!`}
        </p>
      </div>

      <div className="mt-5">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-sm font-bold text-gray-900">모리 스티커</h3>
        ...
        <ul className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            return (
              <li
                ...
                className={`flex min-h-[44px] items-center gap-2 rounded-xl border px-3 py-2 transition-all duration-300 ${
                  meta.className
                } ${isNew ? 'scale-105 ring-2 ring-sagebg' : ''}`}
              >
                <span
                  aria-hidden="true"
          ...
          {Array.from({ length: emptySlotCount }).map((_, index) => (
            <li
              key={`empty-${index}`}
              className="flex min-h-[44px] items-center justify-center rounded-xl border border-dashed border-sagebg bg-sagebg/60 px-3 py-2 text-xs font-semibold text-sage"
            >
              다음 꽃 자리
            </li>
          ))}
          ...
          {hiddenStickerCount > 0 && (
            <li className="flex min-h-[44px] items-center justify-center rounded-xl border border-line bg-gray-50 px-3 py-2 text-sm font-bold text-gray-500">
